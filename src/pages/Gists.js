...
const Gist = () => {
    ...
    const renderGist=useCallback((gist,index)=>(
        <li key={index}>{gist.description ||  "No description"} </li>
    ),[]);


    if(loading){
        return <div className={"reconnect"}>
         <CircularProgress/>
        </div>
    }
    ...
    if(error){
        return <div className={"reconnect"}>
            <>
                <h3>Oops, Error</h3>
                <Button style={{ backgroundColor:"lightblue" , marginTop:"10px"}} onClick={requestGists}>Try again</Button>
            </>
        </div>
    }
